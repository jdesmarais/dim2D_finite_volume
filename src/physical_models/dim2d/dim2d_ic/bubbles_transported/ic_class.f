      ...
      !> the computational domain for two bubbles advected by the flow
      !
      !> @author 
      !> Julien L. Desmarais
      !
      ...
      !
      !> @date
      !> 14_12_2015 - initial version - J.L. Desmarais
      !-----------------------------------------------------------------
      module ic_class
     ...
     $       rkind        
        ...
        type, extends(ic_abstract) :: ic
          ...
          character(18) :: name = 'bubbles_advected' !<@brief name of the initial condition
        ...
        !> \f]
        !> where
        !> \f[ \rho_{\textrm{bubbles}} =
        !> \begin{cases}
        !>      \rho_\textrm{bubble}(\sqrt{(x-x_{C1})^2+(y-y_{C1})^2},2L_i) & \mbox{if} \quad y > 0 \\\
        !>      \rho_\textrm{bubble}(\sqrt{(x-x_{C2})^2+(y-y_{C2})^2},2L_i) & \mbox{if} \quad y \le 0 \\\
        !> \end{cases}
        !> \f]
        !> where \f$(x_{C1}=x_{C2}=0)\f$ and \f$y_{C1}=-y_{C2}=l_\textrm{sep}*L_i+2 L_i\f$, \f$l_\textrm{sep}\f$ is the separation
        !> length between the two bubbles, expressed as a fraction of the width of the interface \f$L_i\f$. The mass
        !> density field for a single bubble is given by:
        ...
        subroutine apply_ic(nodes,x_map,y_map)
          ...
          !set the center of the droplets
          xc1=0.0d0
          xc2=0.0d0
          yc1=0.5d0*li_separation*li+a
          yc2=-yc1
     ...
     $                  x,y,xc1,yc1,a,b,li,dliq,dvap,phase_at_center)
                else
                   nodes(i,j,1) = mass_density_ellipsoid(
     $                  x,y,xc2,yc2,a,b,li,dliq,dvap,phase_at_center)
                end if                   
     ...
     $                  x,y,xc1,yc1,a,b,li,dliq,dvap,
     $                  nodes(i,j,1),T0)

                else
                   nodes(i,j,4) = total_energy_ellipsoid(
     $                  x,y,xc2,yc2,a,b,li,dliq,dvap,
     $                  nodes(i,j,1),T0)
                end if
     ...
     $               0.5d0*nodes(i,j,1)*(velocity_x**2+velocity_y**2)                   
        ...
        !> \begin{pmatrix}
        !> \rho_\textrm{liq}(T_0) \\\
        !> \rho_\textrm{liq}(T_0) u_0 \\\
        !> \rho_\textrm{liq}(T_0) v_0 \\\
        !> \rho_\textrm{liq}(T_0) \left[ \frac{8}{3} c_v T_0 - 3 \rho_\textrm{liq}(T_0) \right]
        !> + \frac{1}{2} \rho_\textrm{liq}(T_0) \left( u_0^2 + v_0^2 \right)
        !> \end{pmatrix}
        !> \f]
        !
        !> @date
