      ...
      module fv_operators_class
        ...
        use cg_operators_class, only : cg_operatprs
     ...
     $       time_dev)
            ...
            allocate(flux_x(nx,ny,ne))
            allocate(flux_y(nx,ny,ne))
            ...
            do k=1, ne
               do j=1+bc_size, ny-bc_size
                  do i=1+bc_size, nx-bc_size
                     time_dev(i,j,k)=
     $                    (flux_x(i+1,j,k)-flux_x(i,j,k))/field_used%dx+
     $                    (flux_y(i,j+1,k)-flux_y(i,j,k))/field_used%dy
                  end do
               end do
            end do
