      program test_bf_layer_bc_fluxes
     ...
     $       pmodel_eq
        ...
        test_loc = test_extract_grdpts_to_compute_bc_fluxes(detailled)
        test_validated = test_validated.and.test_loc
        print '(''test_extract_grdpts_to_compute_bc_fluxes: '',L1)', test_loc
        print '()'
     ...
     $       result(test_validated)
          ...
          integer, dimension(2,2,2) :: border_coords2_test
     ...
     $         (/2,2/))
          ...
          ! test with one grid-point computed
     ...
     $            grdpts_needed)
          ...
          ! test with several gridpoints computed
          do k=1,size(pattern,3)
     ...
     $            grdpts_needed)
             ...
             !validation+detailled
             test_loc = is_int_matrix_validated(border_coords,border_coords2_test(:,:,k),detailled)
             if(detailled.and.(.not.test_loc)) then
                print '(''borders_coords failed'')'
             end if
             test_validated = test_validated.and.test_loc
     ...
     $            cpt_coords)
        ...
        function test_extract_grdpts_to_compute_bc_fluxes(
     $     detailled)
     $     result(test_validated)
     ...
     $         tmp_nodes, tmp_nodes_test, detailled)
        ...
        end function test_extract_grdpts_to_compute_bc_fluxes

      end program test_bf_layer_bc_fluxes
