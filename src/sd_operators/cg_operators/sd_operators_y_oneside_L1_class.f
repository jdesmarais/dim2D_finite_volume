     ...
     $                                     get_secondary_var
        ...
        type, extends(sd_operators) :: sd_operators_y_oneside_L1
          ...
          procedure, nopass :: dfdx_nl     => dfdx_y_oneside_L1_nl

          procedure, nopass :: g           => g_y_oneside_L1
     ...
     $     result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var = 1.0d0/dx*(
     $            -proc(nodes,i-1,j,dx,dy,gradient_x_interior,gradient_y_y_oneside_L1)
     $            +proc(nodes,i  ,j,dx,dy,gradient_x_interior,gradient_y_y_oneside_L1)
     $            )
          else
             var = 1./dx*(
     $            -proc(nodes,i-1,j,dx,dy,gradient_x_interior,gradient_y_y_oneside_L1)
     ...
     $            )
        ...
        end function dfdx_y_oneside_L1_nl
        ...
        !> 5 u_{i,j-1} + 7 u_{i,j} + u_{i,j+1} - u_{i,j+2}
        !> \right)\f$
        !
        !> @date
        !> 30_07_2014 - initial version  - J.L. Desmarais
        ...
        !> data evaluated at [i,j]
        !---------------------------------------------------------------
        function g_y_oneside_L1(nodes,i,j,proc) result(var)
     ...
     $            -     proc(nodes,i,j+2))
        ...
        !> data evaluated at [i,j]
        !---------------------------------------------------------------
        function dgdx_y_oneside_L1(nodes,i,j,proc,dx)
     $     result(var)
     ...
     $            -      proc(nodes,i+1,j+2))
        ...
        end function dgdx_y_oneside_L1
        ...
        !> ,j-\frac{1}{2}}= \frac{1}{\Delta x}(-u_{i,j}+u_{i-1,j})\f$
        !
        !> @date
