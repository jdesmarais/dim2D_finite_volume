     ...
     $                                 get_secondary_var
        ...
        type, extends(sd_operators) :: sd_operators_x_oneside_L1
          ...
          procedure, nopass :: f           => f_x_oneside_L1
          procedure, nopass :: dfdx_nl     => dfdx_x_oneside_L1_nl
          procedure, nopass :: dfdy        => dfdy_x_oneside_L1
          procedure, nopass :: d2fdx2      => d2fdx2_x_oneside_L1
          procedure, nopass :: d2fdy2      => d2fdy2_x_oneside_L1

          procedure, nopass :: dgdy_nl     => dgdy_x_oneside_L1_nl
        ...
        !> 5 u_{i-1,j} + 7 u_{i,j} + u_{i+1,j} - u_{i+2,j}
        !> \right)\f$
        !
        !> @date
        !> 30_07_2014 - initial version  - J.L. Desmarais
        ...
        !---------------------------------------------------------------
        function f_x_oneside_L1(nodes,i,j,proc) result(var)
     ...
     $            +proc(nodes,i  ,j,dx,dy,gradient_x_x_oneside_L1, gradient_y_interior))
          else
             var = 1./dx*(
     $            -proc(nodes,i-1,j,dx,dy,gradient_x_x_oneside_L0,gradient_y_interior)
     $            +proc(nodes,i  ,j,dx,dy,gradient_x_x_oneside_L1,gradient_y_interior))
        ...
        !> data evaluated at [i,j]
        !---------------------------------------------------------------
        function dfdy_x_oneside_L1(nodes,i,j,proc,dy)
     $     result(var)
     ...
     $            -      proc(nodes,i+2,j+1))
        ...
        end function dfdy_x_oneside_L1
     ...
     $            )
        ...
        end function d2fdy2_x_oneside_L1
     ...
     $     result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var = 1.0d0/dy*(
     $            -proc(nodes,i,j-1,dx,dy,gradient_x_x_oneside_L1,gradient_y_interior)
     $            +proc(nodes,i,j  ,dx,dy,gradient_x_x_oneside_L1,gradient_y_interior)
     $            )
          else
             var = 1./dy*(
     ...
     $            )
        ...
        end function dgdy_x_oneside_L1_nl
