     ...
     $       sd_operators
        ...
        !> -\frac{1}{2} u_{i-2,j} + \frac{3}{2} u_{i-1,j}\f$
        !
        !> @date
        ...
        function f_x_oneside_R0(nodes,i,j,proc) result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var =-0.5d0*proc(nodes,i-2,j)
     $            +1.5d0*proc(nodes,i-1,j)
          else
             var =-0.5*proc(nodes,i-2,j)
     ...
     $            -3.0d0*proc(nodes,i-2,j,dx,dy,gradient_x_interior     ,gradient_y_interior)
     $            +2.0d0*proc(nodes,i-1,j,dx,dy,gradient_x_x_oneside_R0, gradient_y_interior))
          else
             var = 1.0/dx*(
     $                 proc(nodes,i-3,j,dx,dy,gradient_x_interior     ,gradient_y_interior)
     $            -3.0*proc(nodes,i-2,j,dx,dy,gradient_x_interior     ,gradient_y_interior)
     ...
     $     result(var)
          ...
          if(rkind.eq.8) then
             !TAG INLINE
             var = 1.0d0/(2.0d0*dy)*(
     $            + 0.5d0*proc(nodes,i-2,j-1)
     $            - 1.5d0*proc(nodes,i-1,j-1)
     $            - 0.5d0*proc(nodes,i-2,j+1)
     $            + 1.5d0*proc(nodes,i-1,j+1))
          else
             var = 1.0d0/(2.0d0*dy)*(
     ...
     $            )
             ...
             var = (1.0/dy**2)*
     $            (
     $            -0.5*proc(nodes,i-2,j-1)
     $            +1.5*proc(nodes,i-1,j-1)
     $            -2.0*(
