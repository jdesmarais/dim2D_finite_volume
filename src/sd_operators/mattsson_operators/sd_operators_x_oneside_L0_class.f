     ...
     $       sd_operators
        ...
        type, extends(sd_operators) :: sd_operators_x_oneside_L0
          ...
          procedure, nopass :: dgdy_nl     => dgdy_x_oneside_L0_nl
     ...
     $            -    proc(nodes,i+2,j))
        ...
        function dfdx_x_oneside_L0_nl(nodes,i,j,proc,dx,dy) result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var = 1.0d0/dx*(
     $            -2.0d0*proc(nodes,i  ,j,dx,dy,gradient_x_x_oneside_L0,gradient_y_interior)
     $            +3.0d0*proc(nodes,i+1,j,dx,dy,gradient_x_interior    ,gradient_y_interior)
     $            -      proc(nodes,i+2,j,dx,dy,gradient_x_interior    ,gradient_y_interior))
          else
             var = 1.0/dx*(
     $            -2.0*proc(nodes,i  ,j,dx,dy,gradient_x_x_oneside_L0,gradient_y_interior)
     $            +3.0*proc(nodes,i+1,j,dx,dy,gradient_x_interior    ,gradient_y_interior)
     $            -    proc(nodes,i+2,j,dx,dy,gradient_x_interior    ,gradient_y_interior))
          end if
     ...
     $     result(var)
          ...
          if(rkind.eq.8) then
             !TAG INLINE
             var = 1.0d0/(2.0d0*dy)*(
     $            - 1.5d0*proc(nodes,i  ,j-1)
     $            + 0.5d0*proc(nodes,i+1,j-1)
     $            + 1.5d0*proc(nodes,i  ,j+1)
     $            - 0.5d0*proc(nodes,i+1,j+1))
          else
             var = 1.0/(2.0*dy)*(
     $            - 1.5*proc(nodes,i  ,j-1)
     ...
     $     result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var = 1.0d0/dy*(
     $            -proc(nodes,i,j-1,dx,dy,gradient_x_x_oneside_L0,gradient_y_interior)
     $            +proc(nodes,i,j  ,dx,dy,gradient_x_x_oneside_L0,gradient_y_interior)
     $            )
          else
             var = 1./dy*(
     $            -proc(nodes,i,j-1,dx,dy,gradient_x_x_oneside_L0,gradient_y_interior)
     ...
     $            )
        ...
        end function dgdy_x_oneside_L0_nl
