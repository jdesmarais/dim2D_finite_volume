     ...
     $                                        get_secondary_var
        ...
        private
        public :: sd_operators_y_oneside_L0


        !> @class sd_operators_y_oneside_L0
        !> class encapsulating 
        !
        !> @param get_bc_size
        ...
        type, extends(sd_operators) :: sd_operators_y_oneside_L0
          ...
          procedure, nopass :: d2fdy2      => d2fdy2_y_oneside_L0
          procedure, nopass :: d2fdxdy     => d2fdxdy_y_oneside_L0

          procedure, nopass :: g           => g_y_oneside_L0
          procedure, nopass :: dgdx        => dgdx_y_oneside_L0
          procedure, nopass :: dgdy        => dgdy_y_oneside_L0
          procedure, nopass :: dgdy_nl     => dgdy_y_oneside_L0_nl
          procedure, nopass :: d2gdx2      => d2gdx2_y_oneside_L0
          procedure, nopass :: d2gdy2      => d2gdy2_y_oneside_L0
          procedure, nopass :: d2gdxdy     => d2gdxdy_y_oneside_L0
     ...
     $     result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var = 1.d0/dx*(
     $            -proc(nodes,i-1,j,dx,dy,gradient_x_interior,gradient_y_y_oneside_L0)
     $            +proc(nodes,i  ,j,dx,dy,gradient_x_interior,gradient_y_y_oneside_L0))
          else
             var = 1./dx*(
     $            -proc(nodes,i-1,j,dx,dy,gradient_x_interior,gradient_y_y_oneside_L0)
     $            +proc(nodes,i  ,j,dx,dy,gradient_x_interior,gradient_y_y_oneside_L0))
        ...
        end function dfdx_y_oneside_L0_nl
     ...
     $            + 0.5*proc(nodes,i  ,j+1))
        ...
        end function dfdy_y_oneside_L0
        ...
        !> u_{i-\frac{1}{2},j}- 2 u_{i-\frac{1}{2}+1,j+1}
        !> + u_{i-\frac{1}{2},j+2}) \f$
        !
        !> @date
        !> 30_07_2014 - initial version - J.L. Desmarais
     ...
     $            +0.5*proc(nodes,i  ,j+1)
     $            )
     $            +0.5*proc(nodes,i-1,j+2)
     $            +0.5*proc(nodes,i  ,j+2)
     $            )
          end if

        end function d2fdy2_y_oneside_L0
        ...
        !> (-\frac{\partial u}{\partial x}\bigg|_{i-\frac{1}{2},j} +
     ...
     $            1.0/(dx*dy)
        ...
        end function d2fdxdy_y_oneside_L0
        ...
        !>\frac{3}{2} u_{i,j} - \frac{1}{2} u_{i,j+1}\f$
        !
        !> @date
        !> 29_07_2014 - initial version  - J.L. Desmarais
        !
        ...
        !> data evaluated at [i,j]
        !---------------------------------------------------------------
        function g_y_oneside_L0(nodes,i,j,proc) result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var = 1.5d0*proc(nodes,i,j)
     $            -0.5d0*proc(nodes,i,j+1)
          else
             var = 1.5*proc(nodes,i,j)
     $            -0.5*proc(nodes,i,j+1)
          end if

        end function g_y_oneside_L0
        ...
        !
        !> @brief
        !> compute\f$ \frac{\partial u}{\partial y}\bigg|_{i,j-\frac{1}{2}}
        !> = \frac{1}{\Delta y}(- 2 u_{i,j} + 3 u_{i,j+1} - u_{i,j+2}) \f$
        !
        !> @date
        !> 30_07_2014 - initial version  - J.L. Desmarais
        !
        ...
        !> data evaluated at [i,j]
        !---------------------------------------------------------------
        function dgdy_y_oneside_L0(nodes,i,j,proc,dy) result(var)
     ...
     $            -    proc(nodes,i,j+2))
        ...
        end function dgdy_y_oneside_L0
        ...
        !> data evaluated at [i,j]
        !---------------------------------------------------------------
        function dgdy_y_oneside_L0_nl(nodes,i,j,proc,dx,dy) result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var = 1.0d0/dy*(
     $            -2.0d0*proc(nodes,i,j  ,dx,dy,gradient_x_interior,gradient_y_y_oneside_L0)
     $            +3.0d0*proc(nodes,i,j+1,dx,dy,gradient_x_interior,gradient_y_interior    )
     $            -      proc(nodes,i,j+2,dx,dy,gradient_x_interior,gradient_y_interior    ))
          else
             var = 1.0/dy*(
     $            -2.0*proc(nodes,i,j  ,dx,dy,gradient_x_interior,gradient_y_y_oneside_L0)
     $            +3.0*proc(nodes,i,j+1,dx,dy,gradient_x_interior,gradient_y_interior    )
     $            -    proc(nodes,i,j+2,dx,dy,gradient_x_interior,gradient_y_interior    ))
          end if

        end function dgdy_y_oneside_L0_nl
        ...
        !
        !> @brief
        !> compute \f$ \frac{\partial u}{\partial x}\bigg|
        ...
        !> data evaluated at [i,j]
        !---------------------------------------------------------------
        function dgdx_y_oneside_L0(nodes,i,j,proc,dx)
     $     result(var)
          ...
          if(rkind.eq.8) then
             !TAG INLINE
             var = 1.0d0/(2.0d0*dx)*(
     $            - 1.5d0*proc(nodes,i-1,j  )
     $            + 0.5d0*proc(nodes,i-1,j+1)
     $            + 1.5d0*proc(nodes,i+1,j  )
     $            - 0.5d0*proc(nodes,i+1,j+1))
          else
             var = 1.0/(2.0*dx)*(
     $            - 1.5*proc(nodes,i-1,j  )
     $            + 0.5*proc(nodes,i-1,j+1)
     $            + 1.5*proc(nodes,i+1,j  )
     $            - 0.5*proc(nodes,i+1,j+1))
          end if

        end function dgdx_y_oneside_L0
        ...
        !> _{i,j-\frac{1}{2}}= \frac{1}{2 {\Delta y}^2}(3 u_{i,j} -
        !> 5 u_{i,j+1} - u_{i,j+2} + 5 u_{i,j+3} -2 u_{i,j+4}) \f$
        !
        !> @date
        !> 30_07_2014 - initial version - J.L. Desmarais
        !
     ...
     $            -    proc(nodes,i,j+2)
     $            +5.0*proc(nodes,i,j+3)
     $            -2.0*proc(nodes,i,j+4)
     $            )
          end if

        end function d2gdy2_y_oneside_L0
     ...
     $            )
        ...
        end function d2gdx2_y_oneside_L0
        ...
        !> (-\frac{\partial u}{\partial x} \bigg|_{i-1,j-\frac{1}{2}} +
        !> \frac{\partial u}{\partial x} \bigg|_{i+1,j-\frac{1}{2}}) \f$
        !
        !> @date
        !> 30_07_2014 - initial version - J.L. Desmarais
        !
     ...
     $     result(var)
          ...
          if(rkind.eq.8) then
             ...
             !TAG INLINE
             var =( 2.0d0*proc(nodes,i-1,j  )
     $            - 3.0d0*proc(nodes,i-1,j+1)
     $            +       proc(nodes,i-1,j+2)
     $            - 2.0d0*proc(nodes,i+1,j  )
     $            + 3.0d0*proc(nodes,i+1,j+1)
     $            -       proc(nodes,i+1,j+2))*
     $            0.5d0/(dy*dx)
          else
             var =( 2.0*proc(nodes,i-1,j  )
     $            - 3.0*proc(nodes,i-1,j+1)
     $            +     proc(nodes,i-1,j+2)
     $            - 2.0*proc(nodes,i+1,j  )
     $            + 3.0*proc(nodes,i+1,j+1)
     $            -     proc(nodes,i+1,j+2))*
     $            0.5/(dy*dx)
          end if

        end function d2gdxdy_y_oneside_L0        
        
      end module sd_operators_y_oneside_L0_class
