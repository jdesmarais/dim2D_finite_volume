...
def extract_interior_data(
    sizes_filename, grdptid_filename, nodes_filename):
...
def extract_interior_data_with_maps(
    sizes_filename,
    grdptid_filename,
    x_map_filename,
    y_map_filename,
    ...
    #return the interior point data
    return [sizes, grdpts_id, x_map, y_map, nodes]
...
def extract_bf_layer_data_with_maps(
    sizes_filename,
    x_map_filename,
    y_map_filename,
    nodes_filename,
    ...
    #return the buffer layer data
    return [sizes, x_map, y_map, nodes, grdpt_id]
...
def make_matrix_for_all_bf_layers_with_maps(
    interior_size_filename,
    interior_grdptsid_filename,
    ...
    [nodes_sizes,
     ...
     x_map,
     y_map,
     nodes] = extract_interior_data_with_maps(
        interior_size_filename,
        interior_grdptsid_filename,
        interior_x_map_filename,
        interior_y_map_filename,
    ...
    #allocation of the large matrices for the x_map, y_map, nodes
    #and the grdptid
    lm_x_map = np.empty([lm_size_y, lm_size_x])
    lm_x_map.fill(backgrd_pt)
    ...
    lm_x_map[
        j_match,
        i_match:i_match+nodes_size_x] = x_map[:]
        ...
        for i in range(1,nb_sublayers+1):
            ...
            if(os.path.isfile(filename)):
                ...
                [lm_x_map,
                 lm_y_map,
                 lm_nodes,
                 lm_grdptid] = fill_bf_layer_data_with_maps(
                    folder_path,
                    mainlayer,
                    i,
                    suffix_size,
    ...
    return [lm_x_map,lm_y_map,lm_nodes,lm_grdptid,margin]
...
def fill_bf_layer_data_with_maps(
    folder_path,
    mainlayer,
    sublayer,
    suffix_size,
    ...
    #determine the filenames
    size_filename    = folder_path+'/'+mainlayer+str(sublayer)+suffix_size
    x_map_filename   = folder_path+'/'+mainlayer+str(sublayer)+suffix_x_map
    y_map_filename   = folder_path+'/'+mainlayer+str(sublayer)+suffix_y_map
    nodes_filename   = folder_path+'/'+mainlayer+str(sublayer)+suffix_nodes
    grdptid_filename = folder_path+'/'+mainlayer+str(sublayer)+suffix_grdptid
    ...
    [sizes,
     ...
     y_map,
     nodes,
     grdptid] = extract_bf_layer_data_with_maps(
            size_filename,
            x_map_filename,
            y_map_filename,
            nodes_filename,
    ...
    if(continuous):
        selective_copy_with_maps(
            mainlayer,
            lm_x_map, lm_y_map, lm_nodes, lm_grdptid,
            x_map, y_map, nodes, grdptid,
            i_match, j_match,
            bf_layer_size_x, bf_layer_size_y)
    else:
        #print mainlayer
        ...
        lm_x_map[j_match,
                 i_match:i_match+bf_layer_size_x] = x_map[:]

        lm_y_map[j_match:j_match+bf_layer_size_y,
                 i_match] = y_map[:]
    ...
    return [lm_x_map,lm_y_map,lm_nodes,lm_grdptid]
...
def conditional_copy_with_maps(
    lm_x_mpa, lm_y_map, lm_nodes, lm_grdptid,
    x_map, y_map, nodes, grdptid,
    lm_i_min, lm_i_max,
    lm_j_min, lm_j_max,
    i_min, i_max,
    ...
    for (lm_j,j) in zip(range(lm_j_min,lm_j_max), range(j_min,j_max)):
        for (lm_i,i) in zip(range(lm_i_min,lm_i_max), range(i_min,i_max)):
            if(grdptid[j,i]!=no_pt):
                ...
                lm_x_map[lm_j,lm_i]   = x_map[j,i]
                lm_y_map[lm_j,lm_i]   = y_map[j,i]
                lm_grdptid[lm_j,lm_i] = grdptid[j,i]
                lm_nodes[:,lm_j,lm_i] = nodes[:,j,i]
...
def plot_nodes_and_grdptid_with_all_bf_layers_with_maps(
    lm_x_map,
    lm_y_map,
    lm_nodes,
    lm_grdptid):
